...
if (extract($_POST)):
	...
	try {
		...
		$act = $age->setLast($pers, $esp);

		if (!$act['estado']):
			throw new Exception('Error al actualizar las agendas anteriores. ' . $act['msg']);
		endif;
		...
		$bh_ac = $bh->setLast($pers);

		if (!$bh_ac['estado']):
			throw new Exception('Error al actualizar los bloqueos anteriores. ' . $act['msg']);
		endif;
