...
if ($_POST):
    foreach ($_POST as $key => $value):
        ...
        if ($value != '' and $value != '0.00' and $value != '0' and $init != 't'):
            $data[$key] = $value;
        endif;
    endforeach;
    ...
    try {
        ...
        $ins_d = $di->mod($data['id'], $data['idesc'], $data['iobserv'], $date_ini, $date_ter, $justif, $serv, $esp, $_POST['ivacaciones'], $_POST['ipermiso'], $_POST['icongreso'], $_POST['idescanso'], $igeneral, $_SESSION['prm_userid'], $db);

        if (!$ins_d['estado']):
            throw new Exception('Error al editar los datos de la programación. ' . $ins_d['msg']);
        endif;
        
        $del = $dh->delByDist($ins_d['msg'], $db);
                
        if (!$del['estado']):
            throw new Exception('Error al editar-eliminar los datos de la programación. ' . $del['msg']);
        endif;
		...
		foreach($data as $key => $value):
			...
			if ($ind[0] != 'i'):
				switch ($key):
					case 'disp':            $thor = 1; $rend = 0; $obs = ''; $insert = true; break;
					case 'universidad':     $thor = 2; $rend = 0; $obs = ''; $insert = true; break;
					case 'becados':         $thor = 3; $rend = 0; $obs = ''; $insert = true; break;
					default:                break;
				endswitch;
			else:
				$thor = $ind[1];
